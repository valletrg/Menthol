...
impl ServerMessage {
    pub fn decode(code: u32, payload: &mut impl Buf) -> Result<Self, ProtoError> {
        match code {
            ...
            get_peer_address::CODE => Ok(Self::GetPeerAddress(
                get_peer_address::GetPeerAddressResponse::read(payload)?,
            )),
            watch_user::CODE => Ok(Self::WatchUser(watch_user::WatchUserResponse::read(
                payload,
            )?)),
            unwatch_user::CODE => Ok(Self::UnwatchUser),
            get_user_status::CODE => Ok(Self::GetUserStatus(
                get_user_status::GetUserStatusResponse::read(payload)?,
            )),
            say_chatroom::CODE => Ok(Self::SayChatroom(say_chatroom::SayChatroomResponse::read(
                payload,
            )?)),
            join_room::CODE => Ok(Self::JoinRoom(join_room::JoinRoomResponse::read(payload)?)),
            leave_room::CODE => Ok(Self::LeaveRoom),
            user_joined_room::CODE => Ok(Self::UserJoinedRoom(
                user_joined_room::UserJoinedRoom::read(payload)?,
            )),
            user_left_room::CODE => Ok(Self::UserLeftRoom(user_left_room::UserLeftRoom::read(
                payload,
            )?)),
            connect_to_peer::CODE => Ok(Self::ConnectToPeer(
                connect_to_peer::ConnectToPeerResponse::read(payload)?,
            )),
            message_user::CODE => Ok(Self::MessageUser(message_user::MessageUserResponse::read(
                payload,
            )?)),
            message_acked::CODE => Ok(Self::MessageAcked),
            file_search::CODE => Ok(Self::FileSearch(file_search::FileSearchResponse::read(
                payload,
            )?)),
            set_status::CODE => Ok(Self::SetStatus),
            server_ping::CODE => Ok(Self::ServerPing),
            shared_folders_files::CODE => Ok(Self::SharedFoldersFiles),
            get_user_stats::CODE => Ok(Self::GetUserStats(
                get_user_stats::GetUserStatsResponse::read(payload)?,
            )),
            ...
            recommendations::CODE => Ok(Self::Recommendations(
                recommendations::RecommendationsResponse::read(payload)?,
            )),
            global_recommendations::CODE => Ok(Self::GlobalRecommendations(
                global_recommendations::GlobalRecommendationsResponse::read(payload)?,
            )),
            user_interests::CODE => Ok(Self::UserInterests(
                user_interests::UserInterestsResponse::read(payload)?,
            )),
            room_list::CODE => Ok(Self::RoomList(room_list::RoomListResponse::read(payload)?)),
            admin_message::CODE => Ok(Self::AdminMessage(
                admin_message::AdminMessageResponse::read(payload)?,
            )),
            privileged_users::CODE => Ok(Self::PrivilegedUsers(
                privileged_users::PrivilegedUsersResponse::read(payload)?,
            )),
            have_no_parent::CODE => Ok(Self::HaveNoParent),
            parent_min_speed::CODE => Ok(Self::ParentMinSpeed(
                parent_min_speed::ParentMinSpeedResponse::read(payload)?,
            )),
            parent_speed_ratio::CODE => Ok(Self::ParentSpeedRatio(
                parent_speed_ratio::ParentSpeedRatioResponse::read(payload)?,
            )),
            check_privileges::CODE => Ok(Self::CheckPrivileges(
                check_privileges::CheckPrivilegesResponse::read(payload)?,
            )),
            embedded_message::CODE => Ok(Self::EmbeddedMessage(
                embedded_message::EmbeddedMessageResponse::read(payload)?,
            )),
            accept_children::CODE => Ok(Self::AcceptChildren),
            possible_parents::CODE => Ok(Self::PossibleParents(
                possible_parents::PossibleParentsResponse::read(payload)?,
            )),
            wishlist_search::CODE => Ok(Self::WishlistSearch),
            wishlist_interval::CODE => Ok(Self::WishlistInterval(
                wishlist_interval::WishlistInterval::read(payload)?,
            )),
            similar_users::CODE => Ok(Self::SimilarUsers(
                similar_users::SimilarUsersResponse::read(payload)?,
            )),
            room_members::CODE => Ok(Self::RoomMembers(room_members::RoomMembersResponse::read(
                payload,
            )?)),
            add_room_member::CODE => Ok(Self::AddRoomMember(
                add_room_member::AddRoomMemberResponse::read(payload)?,
            )),
            remove_room_member::CODE => Ok(Self::RemoveRoomMember(
                remove_room_member::RemoveRoomMemberResponse::read(payload)?,
            )),
            ...
            reset_distributed::CODE => Ok(Self::ResetDistributed(
                reset_distributed::ResetDistributed::read(payload)?,
            )),
            global_room_message::CODE => Ok(Self::GlobalRoomMessage(
                global_room_message::GlobalRoomMessageResponse::read(payload)?,
            )),
            excluded_search_phrases::CODE => Ok(Self::ExcludedSearchPhrases(
                excluded_search_phrases::ExcludedSearchPhrasesResponse::read(payload)?,
            )),
            cant_connect_to_peer::CODE => Ok(Self::CantConnectToPeer(
                cant_connect_to_peer::CantConnectToPeerResponse::read(payload)?,
            )),
            cant_create_room::CODE => Ok(Self::CantCreateRoom(
                cant_create_room::CantCreateRoom::read(payload)?,
            )),
